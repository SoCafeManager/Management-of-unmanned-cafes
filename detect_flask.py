...
#model_path = r"C:\Users\USER\Documents\KTH_4grade\yolov5_KTH_site\stream\yolo\runs\cafe23_results\weights\best.pt"
# Load custom model
model = torch.hub.load('ultralytics/yolov5', 'custom', path=model_path, force_reload=True)

# Time variables
last_snapshot_time = 0  # Last snapshot time initialized to 0 or a past timestamp
snapshot_interval = 30 * 60  # Snapshot interval set to 30 minutes (in seconds)
    ...
    while True:
        ...
        if not success:
            ...
            try:
                ...
                if any(model.names[int(cls)] in ['person','cellphone', 'wallet', 'bag'] for *xyxy, conf, cls in results.xyxy[0]):
